...
def tensor_to_image(tensor):
    ...
    """
    image = tensor.permute(0, 2, 3, 1).cpu().numpy().squeeze(0)
    image = (image + 1) * 127.5
...
def save_singer_image(watermarking_images,folder):
    watermarking_images=watermarking_images[:watermarking_images.shape[0],:,:,:]
    watermarking_images=(watermarking_images+1)/2
    for watermarking_image in watermarking_images:
        filename=os.path.join(folder,'watermarking_image'.format(watermarking_image))
        torchvision.utils.save_image(watermarking_image,filename)
...
def get_data_loaders(hidden_config: HiDDenConfiguration, train_options: TrainingOptions):
    ...
    validation_images = datasets.ImageFolder(train_options.validation_folder, data_transforms['val_origin'])
...
def majority_vote(watermark):
    # watermark:(5,30):5 is the number of watermark, and 30 is the message's length
    w_batch=watermark.shape[0]
    w_length=watermark.shape[1]
    temp_watermark=np.zeros((1,w_length))
    for i in range(w_batch):
        temp_watermark=temp_watermark+watermark[i]
    temp_value=w_batch/2
    out=(temp_watermark>temp_value).astype(int)
    final_watermark = out.squeeze()
