...
def train(model: Hidden,
    ...
    print_each = 10
    images_to_save = 8
    saved_images_size = (128, 128)  #save cover image\ encodered image \and they are diff
    ...
    for epoch in range(train_options.start_epoch, train_options.number_of_epochs + 1):
        ...
        for image, _ in train_data:
            cover_image = image.to(device)
            # message = torch.Tensor(np.random.choice([0, 1], (image.shape[0], hidden_config.message_length))).to(device)
            noise_image = image.to(device)
            losses, _ = model.train_on_batch([cover_image,noise_image])
            # print("losses",losses)
        ...
        for image, _ in val_data:
            ...
            noise_image = image.to(device)
            # message = torch.Tensor(np.random.choice([0, 1], (image.shape[0], hidden_config.message_length))).to(device)
            # losses, (encoded_images, noised_images, decoded_tensors) = model.validate_on_batch([cover_image,noise_image])
            losses,(cover_zw,noise_zw)=model.validate_on_batch([cover_image,noise_image])
            for name, loss in losses.items():
