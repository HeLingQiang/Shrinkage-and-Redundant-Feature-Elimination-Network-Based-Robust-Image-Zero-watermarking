...
def parse_Painting(Painting_commond):
    matches=re.match(r'Painting\((\d+\.*\d*)\)',Painting_commond)
    values=matches.groups()[0]
    return Painting(values)
class NoiseArgParser(argparse.Action):
    ...
    def __call__(self, parser, namespace, values,
        ...
        for command in split_commands:
            ...
            elif command[:len('Painting')]=='Painting':
                layers.append(parse_Painting(command))
